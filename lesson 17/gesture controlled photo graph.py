...
pairs = {"middle": ("SEPTA", "NEGATIVE"), "ring": ("BLUR", "GLITCH"), "pinky": ("EDGE", "CARTOON")}
st = {k:0 for k in pairs};
cur = "SEPIA"
DEB, CAP, TT, TP = 0.6, 1.2, 30, 20
la = lc = 0
pinch_on = False
MAIN, POP = "Gesuture based controlled photo app", "Capture (ESC/Close to resume)"
paused = False;
freeze = None;
SEPTA_M = np.array([[0.272, 0.534, 0.131], [0.349, 0.686, 0.168], [0.393, 0.769, 0.189]])

def apply(img, t):
    if t == "SEPIA": return np.clip(cv2.transform(img, SEPTA_M),0, 255).astype(np.uint8)
    ...
    if t == "CARTOON":
        ...
        e = cv2.adaptiveThreshold(cv2.medianBlur(g, 7), 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 9, 10)
...
cap.release(0); cv2.destroyAllWindows(); hands.close()