...
try:
    ...
    vol = dev.EndpointVolume.QueryInterface(IAudioEndpointVolume)
    minv, maxv = vol.GetVolumeRange()[:2]
except Exception as e:
    print(f"Pycaw Error: {e}"); exit()

cap = cv2.VideoCapture(0)
if not cap.isOpened(): print("Error: Camera not found"); exit()
...
while True:
    ...
    if res.multi_hand_landmarks and res.multi_handedness:
        for i, hand in enumerate(res.multi_hand_landmarks):
            ...
            dist = float(mp.hypot(ip[0] - tp[0], ip[1] - tp[1]))

            if label == "Left":
                v = np.interp(dist, [30,300], [minv, maxv])
                try: vol.SetMasterVolumeLevel(v,None)
